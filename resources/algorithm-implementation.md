...
For an overview of the per-product position limits, please refer to the ‘Rounds’ section. The position limits are listed per round.
...
Note: the [datamodel.py](http://datamodel.py) file from which the classes are imported is provided in Appendix B. The code can also be used to test algorithms locally.
